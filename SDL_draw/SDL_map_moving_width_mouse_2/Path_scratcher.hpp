...
public:
  ...
  void next_level(const int & i) { // i: direction give better priority to going strait instead of diagonally      
    level += (dir == 8 ? (i % 2 == 0 ? 10 : 14) : 10);
  }
  
  const int & estimate(const int & xDest, const int & yDest) const { // Estimation function for the remaining distance to the goal.
    static int xd, yd, d;
    xd = xDest - pos_x;
    yd = yDest - pos_y;  
