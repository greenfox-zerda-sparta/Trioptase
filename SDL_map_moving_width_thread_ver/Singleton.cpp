...
void Singleton::print_smooth_route() {
  if (!this->smooth_route.empty()) {
    for (int i = 0; i < this->smooth_route.size(); i++) {
      std::cout << i << ". x = " << this->smooth_route[i].first;
      std::cout << ", x = " << this->smooth_route[i].second << " |\n";
    }
  }
}
